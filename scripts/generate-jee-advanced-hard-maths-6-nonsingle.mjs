 ...
 *   4) Fill until 6 locked (or fill-round cap)
 *   5) Save artifacts
 *
...
// Explanation expand burns a full Gemini call per locked Q and often fails JSON —
// OFF by default (use --expand-explanations=1 only when polishing a final bank).
const EXPAND_EXPLANATIONS =
    argv["expand-explanations"] === "1" ||
    argv["expand-explanations"] === true ||
    process.env.JEE_ADV_EXPAND_EXPLANATIONS === "1";
...
const callGeminiJson = async (prompt, { kind = "generate" } = {}) => {
    ...
    console.log(
        `  [gemini-${kind}] call #${stats.geminiGenCalls} (token-cost: generation)`
    );
...
const shouldCallSolverB = ({
    ...
    if (type === "integer" || type === "numerical") {
        return { run: true, reason: "type=integer (high algebra risk)" };
    }
    if (type === "match") {
        return { run: true, reason: "type=match" };
    }
...
const dualCallSolvers = async (
    ...
    if (openaiCircuitOpen) {
        // Prefer generator as second vote when OpenAI dead (no dual-Gemini)
        if (genKeyNorm != null && String(genKeyNorm).length) {
            console.log(`  [lock-B] using generator key (circuit open)`);
            return {
                a,
                b: { _fromGenerator: true, key: genKeyNorm, confidence: 0.7 },
                mode: "A+generator",
                ranB: true,
                skipReason: null,
            };
        }
        console.log(`  [lock-B] Gemini once as B (no generator key)`);
...
const expandExplanation = async (q) => {
    ...
    console.log(
        `  [expand] Gemini explanation #${stats.geminiExpandCalls} (optional — costs tokens)`
    );
    ...
    const typeRules =
        type === "multiple" || type === "multi"
            ? `MULTI-CORRECT: For EACH option A–D state TRUE/FALSE with a 1–2 sentence reason. End with "Therefore correct option(s): ${lockedKey}".`
            : type === "match"
              ? `MATCH: Solve EACH List-I item separately (numbered 1–4) with full derivation, then show how they assemble into arrangement ${lockedKey}.`
              : type === "integer"
                ? `INTEGER: Full derivation with intermediate values. Last line: "FINAL_ANSWER: ${lockedKey}".`
                : `Full derivation ending with the locked key ${lockedKey}.`;

    const prompt = `You are a JEE Advanced solution author. The answer key is ALREADY LOCKED — do not change it.

Write a COMPLETE student-facing solution (minimum 10 sentences / 8 bullet steps).

LOCKED KEY: ${lockedKey}
${typeRules}

BANNED phrases: "using the standard formula", "it is well known", "obviously", "clearly" without proof.
...
Prior solver notes (may be incomplete — expand fully):
${JSON.stringify(q._dualA || q._solveSteps || q.explanation || "").slice(0, 1200)}

Return ONLY JSON:
{"explanation": "full multi-paragraph derivation", "solveSteps": ["step1", "step2", "..."]}`;
    ...
    try {
        ...
        if (explanation.length < 200) return q;
        return {
            ...q,
            explanation,
...
**HARDNESS RULES (mandatory):**
- multi_concept: fuse ≥2 techniques; ≥4 solve steps; non-telegraphed stem
- Prefer hard_archetypes; NEVER use banned_easy_templates as main ask
- Single clear numerical ask; answer should be an integer when possible
- Veteran / IIT Advanced caliber only
- FIRST STEP must be non-obvious (invariant / state model / geometry identity / clever sub)
- explanation: full derivation (≥8 steps); derive formulas used; no "standard formula" handwaving
...
**HARDNESS / DESIGN (reject Main-level patterns):**
- Each stem needs a hidden insight / non-obvious first step (invariant, geometry, substitution, state idea).
- Fuse ≥2 techniques; ≥5 solve steps before option judgement.
- BANNED as main ask: pure commuting-matrix entry match; pure dy/dx = (dy/dt)/(dx/dt) picker; sole recall of standard inversion center/radius; one-formula plug-in.
- Prefer: extremum on locus, region mapping, tangent/diameter properties, multi-condition characterization, unexpected identity.
- Options: each must require separate judgement (not 3 trivial + 1 hard). At least 2 options should be true OR carefully false for a common Advanced trap.
- difficultySelfScore must be ≥ 80 (self-reject if only Main-hard).
    ...
    for (const q of questions || []) {
        try {
            ...
            if (dual.agree && dual.key) {
                ...
                phaseStep(
                    "3",
                    `multi LOCKED key=${dual.key} mode=${dual.mode}${EXPAND_EXPLANATIONS ? " → expand…" : ""}`
                );
                locked = await expandExplanation(locked);
                out.push(locked);
    ...
    for (const q of questions) {
        try {
            ...
            if (dual.agree && dual.value != null) {
                ...
                phaseStep(
                    "3",
                    `integer LOCKED ${dual.value} mode=${dual.mode}${EXPAND_EXPLANATIONS ? " → expand…" : ""}`
                );
                locked = await expandExplanation(locked);
                out.push(locked);
            } else if (KEEP_UNVERIFIED) {
    ...
    for (const q of questions) {
        try {
            ...
            if (dual.agree && dual.letter) {
                ...
                phaseStep(
                    "3",
                    `match LOCKED ${dual.letter} mode=${dual.mode}${EXPAND_EXPLANATIONS ? " → expand…" : ""}`
                );
                locked = await expandExplanation(locked);
                out.push(locked);
            } else if (KEEP_UNVERIFIED) {
...
const formatBlock = (q, i) => {
    ...
    const lines = [
        `Q${i + 1}. type=${q.questionType || q._advancedType} · ${q._topicId || ""} ${q._chapter || ""}`,
        `Slot: ${q._conceptSlot || q.conceptSlot || "?"}`,
...
async function main() {
    ...
    console.log(
        `Hard bar : skeletonMin=${process.env.AI_QB_SKELETON_DIFFICULTY_SELF_AUDIT_MIN} · lastAttempt=${process.env.AI_QB_SKELETON_SELF_AUDIT_LAST_ATTEMPT_FLOOR} (≥70 keep)`
    );
    console.log(
        `Speed    : expandExplanations=${EXPAND_EXPLANATIONS ? "ON (costs Gemini)" : "OFF"} · openaiAttempts=${OPENAI_ATTEMPTS} · genRetries=${MAX_RETRIES} · fillRounds=${FILL_ROUNDS}`
    );
    console.log(
        `Token policy: exact counts · OpenAI circuit on 429 · no dual-Gemini · no expand by default`
    );
    console.log("");
    ...
    try {
        ...
        phaseStart("2+3", "Generate non-single types + dual lock + fill", [
            "Stage A generation (Gemini hard) → dual solvers (o4/o3)",
