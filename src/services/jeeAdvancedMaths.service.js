...
export const buildJeeAdvancedDesignQualityBlock = ({
    ...
    return `
**JEE ADVANCED DESIGN QUALITY (mandatory — paper-setter bar):**
...
2. **Multi-layer reasoning** — ≥2 independent techniques fused; intermediate result re-used non-obviously.
3. **Non-telegraphed ask** — do NOT ask only for the quantity that the standard formula names (e.g. "center and radius after inversion"). Prefer: region map, tangent property, extremum on locus, diameter image, relation between two constructions, identity that must be *found*.
4. **Minimal but meaningful calculation** — challenge from structure, not giant expressions.
5. **Explanations must derive** — never "using the standard formula…" alone. Show setup → key observation → steps → why wrong options fail (multi) or each List-I item (match) or full state chain (probability).

DO NOT (quality breaks seen in weak paper):
- Commuting-matrix compare-entries with no second layer (Main linear algebra drill).
- Pure dy/dx = (dy/dt)/(dx/dt) option pick with no cusp/loop/area/fixed-point/curvature.
- Single formula recall (standard inversion center/radius only).
- Stretch difficulty only by longer arithmetic / denser LaTeX.
- One-line explanations that state the answer without the core derivation.

Self-check before emit (reject & rewrite if any fail):
- [ ] Would a top Main scorer still need a non-obvious idea? If no → rewrite.
- [ ] Is there at least one "why this approach?" moment? If no → rewrite.
- [ ] explanation ≥ 8 sentences with explicit intermediate results? If no → expand.
- [ ] For multi: every option judged true/false with a reason? If no → expand.
- [ ] For match: each List-I row solved (not only final arrangement)? If no → expand.
`;
};
